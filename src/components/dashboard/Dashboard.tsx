...
import type { DashboardData, WeeklyTrend, StockoutWeekPoint, WarrantyMonthlyChart, PieSlice, WarrantyReasonTrendSeries, FillRateBreakdown, CommissionProgress } from "@/lib/dashboard";
...
export function Dashboard({
  ...
  return (
    <div>
      ...
      {merchandisePayments && (
        <Link
          href={merchandisePayments.href}
          className={`flex items-center gap-3 rounded-md p-4.5 mb-4 border hover:border-blue ${
            merchandisePayments.overdue ? "bg-red/10 border-red/30" : "bg-surface border-rule"
          }`}
        >
          <div className="w-9 h-9 rounded-md bg-blue/15 flex items-center justify-center shrink-0">
            <Package size={17} className="text-blue" />
          </div>
