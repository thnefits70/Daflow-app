    ...
    <div className="flex flex-col gap-2.5">
      {rows.map((r) => (
        <div key={r.id} className="bg-surface border border-rule rounded-md p-4">
          <div className="text-[14px] font-bold mb-0.5">{r.catalogItem.name}</div>
          <div className="text-[11.5px] text-steel mb-3">{r.supplier.name} — se pidieron y pagaron {r.quantity} un. · ${r.totalCost.toFixed(2)}</div>
          ...
          {openId === r.id ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" className="rounded border border-green bg-green px-3.5 py-1.5 text-[12.5px] font-semibold text-white cursor-pointer" onClick={() => { setOpenId(r.id); setErr(""); }}>
                ✓ Confirmar que llegó
              </button>
            </div>
          )}
