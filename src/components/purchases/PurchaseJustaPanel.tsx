...
export function PurchaseJustaPanel() {
  ...
  async function markUploaded(item: JustaItem) {
    ...
    setItems((its) => (its ?? []).filter((i) => !(i.kind === item.kind && i.id === item.id)));
  }
            ...
            {dayItems.map((it) => (
              <div key={`${it.kind}-${it.id}`} className="flex items-center justify-between gap-3 bg-cloud rounded-md px-3 py-2.5">
                ...
                <button
                  ...
                  className="rounded border border-teal bg-teal px-3 py-1.5 text-[12px] font-bold text-navy cursor-pointer disabled:opacity-60 shrink-0"
                  onClick={() => markUploaded(it)}
                >
                  ✓ Subido a JUST
                </button>
              </div>
