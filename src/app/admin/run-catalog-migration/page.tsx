...
// (ver /api/admin/run-catalog-migration). Borrar junto con esa ruta una vez
// confirmado que corrió bien.
export default function RunCatalogMigrationPage() {
  ...
  async function run() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/admin/run-catalog-migration", { method: "POST" });
  ...
  return (
    <div className="max-w-lg">
      ...
      <p className="text-[13px] text-steel mb-4">
        Crea la tabla de avisos de producto faltante y las columnas nuevas de Compras Personales. Es seguro darle click más de una vez — si ya se aplicó, no hace nada.
      </p>
