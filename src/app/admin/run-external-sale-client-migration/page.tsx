...
// (ver /api/admin/run-external-sale-client-migration). Borrar junto con esa
// ruta una vez confirmado que corrió bien.
export default function RunExternalSaleClientMigrationPage() {
  ...
  async function run() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/admin/run-external-sale-client-migration", { method: "POST" });
  ...
  return (
    <div className="max-w-lg">
      ...
      <p className="text-[13px] text-steel mb-4">
        Agrega las columnas de nombre y celular del cliente a Ventas Externas. Es seguro darle click más de una vez — si ya se aplicó, no hace nada.
      </p>
