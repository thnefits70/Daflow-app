...
// Mismo patrón que /transfer/route.ts, pero para PayrollIessTransfer — un
// registro totalmente aparte del de nómina (ver el modelo en schema.prisma).
export async function GET(_req: NextRequest, { params }: { params: Promise<{ period: string }> }) {
  ...
  const payrollPeriod = await prisma.payrollPeriod.findUnique({
    where: { period },
    include: { iessTransfer: { include: { confirmedWithoutProofBy: { select: { name: true } } } } },
  });
  if (!payrollPeriod?.iessTransfer) return NextResponse.json(null);

  const transfer = payrollPeriod.iessTransfer;
  const account =
