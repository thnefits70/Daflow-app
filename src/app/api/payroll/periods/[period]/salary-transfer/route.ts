...
// Mismo patrón que /transfer/route.ts e /iess-transfer/route.ts, pero para
// PayrollNairobySalaryTransfer — un tercer registro totalmente aparte (ver
// el modelo en schema.prisma).
export async function GET(_req: NextRequest, { params }: { params: Promise<{ period: string }> }) {
  ...
  if (!payrollPeriod?.nairobySalaryTransfer) return NextResponse.json(null);

  const transfer = payrollPeriod.nairobySalaryTransfer;
  const account =
