...
import { readNairobySalaryTransferProof } from "@/lib/payrollTransferAi";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ period: string }> }) {
  ...
  if (payrollPeriod.nairobySalaryTransfer.status !== "APPROVED") {
    return NextResponse.json({ error: "Primero hay que aprobarla." }, { status: 409 });
  }

  const expectedAmount = payrollPeriod.nairobySalaryTransfer.totalAmount;
  let readAmount: number | null = null;
  let proofNumber: string | null = null;
  try {
    const read = await readNairobySalaryTransferProof({ proofUrl: parsed.data.proofUrl, actorId: session.user.id });
  ...
  if (financeLeadId) {
    await notifyOwner(financeLeadId, {
      title: "✅ El admin ya transfirió tu sueldo",
