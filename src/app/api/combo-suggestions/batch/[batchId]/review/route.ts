  ...
  await prisma.comboSuggestion.updateMany({
    ...
    data:
      parsed.data.action === "approve"
        ? { status: "APROBADO", reviewedById: session.user.id, reviewedAt: new Date() }
        : { status: "RECHAZADO", rejectReason: parsed.data.rejectReason, reviewedById: session.user.id, reviewedAt: new Date() },
  });
