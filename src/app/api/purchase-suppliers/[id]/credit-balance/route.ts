...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [credits, reserved, applied] = await Promise.all([
    getAvailableCreditsForSupplier(id),
    groupId ? getReservedCreditsForGroup(groupId) : Promise.resolve([]),
    groupId ? getAppliedCreditsForGroup(groupId) : Promise.resolve([]),
  ]);
  const balance = credits.reduce((s, c) => s + c.amount, 0);
  return NextResponse.json({ balance, credits, reserved, applied });
}
