...
// Doble confirmación de Nairoby: ya verificó físicamente el listado de la
// semana (cerrada sola el sábado) contra los productos reales guardados en el
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!batch.justWrittenOffAt) return NextResponse.json({ error: "Esta semana todavía no termina — se cierra sola el sábado." }, { status: 409 });
