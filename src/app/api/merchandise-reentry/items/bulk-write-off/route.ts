...
// Confirma de un solo clic la baja consolidada de un producto: mismo efecto
// que "Registrar baja" item por item, pero para todas las unidades dañadas
// y confirmadas de ese nombre de producto a la vez — ver groupWriteOff en
// batches/close/route.ts.
export async function POST(req: Request) {
  ...
  for (const item of items) {
    ...
    if (item.damagedQty <= 0 || item.damageConfirmed !== true) {
      return NextResponse.json({ error: "Hay un producto sin daño confirmado por Daniel." }, { status: 409 });
    }
