...
// formulario con éxito (ver clearDraft en useFormDraft).
...
const putSchema = z.object({ key: z.string().trim().min(1), data: z.unknown() });
...
export async function PUT(req: NextRequest) {
  ...
  await prisma.formDraft.upsert({
    ...
    create: { userId: session.user.id, formKey: parsed.data.key, data: parsed.data.data as object },
