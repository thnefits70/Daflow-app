...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await recordPick({ lotId: id, catalogItemId: parsed.data.catalogItemId, quantity: parsed.data.quantity, userId: dbUserId(session.user.id), onlyAssigned: scope === "ASSIGNED" });
