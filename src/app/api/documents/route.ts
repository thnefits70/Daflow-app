...
export async function GET(req: NextRequest) {
  ...
  if (moduleId) {
    const documents = await prisma.document.findMany({ where: { moduleId }, orderBy: { createdAt: "asc" } });
    return NextResponse.json(documents);
  }
...
export async function POST(req: NextRequest) {
  ...
  // Módulos cae en el mismo grupo admin-only que Leyes y un documento plano
  // — sin flag de escritura delegada, confirmado 2026-07-25.
  ...
  const document = await prisma.document.create({
    data: {
      ...
      deptId: parsed.data.isLaw || parsed.data.moduleId ? null : (parsed.data.deptId ?? null),
