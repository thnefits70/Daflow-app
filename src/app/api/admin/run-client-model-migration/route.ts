...
// TEMPORAL — 2026-08-31: mismo problema que en las migraciones anteriores
// (prisma migrate deploy no puede correr contra el pooler pgbouncer de
// Supabase). Aplica a mano el SQL que crea la tabla Client y la columna
// ExternalSale.clientId, vía el cliente normal de Prisma. Idempotente.
// Borrar junto con la página una vez confirmado que corrió bien.
const MIGRATION_NAME = "20260831164447_external_sale_client_model";
...
export async function POST() {
  ...
  const already = await prisma.$queryRawUnsafe<{ exists: boolean }[]>(
    `SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'Client') as exists`
  );
