...
// nombres de banco ya escritos antes (acá, en cualquiera de las 3 tablas de
// cuentas bancarias) en vez de mantener un catálogo aparte.
export async function GET() {
  const session = await auth();
  if (!session) return NextResponse.json({ error: "No autorizado." }, { status: 401 });

  const [supplierRows, payeeRows, employeeRows] = await Promise.all([
  ...
  const names = Array.from(new Set([...supplierRows, ...payeeRows, ...employeeRows].map((r) => r.bankName))).sort();
