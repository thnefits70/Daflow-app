...
export async function GET() {
  ...
  const orders = await prisma.personalPurchaseOrder.findMany({
    ...
    include: {
      ...
      items: { select: { confirmedProductName: true, employeeProductName: true, quantity: true, livePhotoUrl: true, optionalPhotoUrl: true, confirmedCatalogItem: { select: { justCode: true } } } },
    },
