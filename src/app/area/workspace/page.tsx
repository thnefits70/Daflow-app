...
export default async function WorkspacePage() {
  ...
  const [canSyncAtomData, canUploadLowRotationList, canApproveComboSuggestions, canActOnComboSuggestions, canMarkComboCreatedInDropi] = await Promise.all([
    checkCanSyncAtomData(),
    checkCanUploadLowRotationList(),
    checkCanApproveComboSuggestions(),
    checkCanActOnComboSuggestions(),
    checkCanMarkComboCreatedInDropi(),
  ]);

  // Fase 2 (Análisis de Mercado) — confirmado 2026-09-09, mismo patrón sin
  // dept.code que Sugerencias de Combos: se ve en la propia "Mi área de
  // trabajo" de quien tenga cada rol.
  const [
    canProposeMarketProductFlag,
    canReviewMarketProductFlag,
    canActOnMarketProductReviewFlag,
    canPublishMarketProductFlag,
    canBrandMarketProductFlag,
  ...
  const [canSubmitPurchases, canViewOwnPurchases, canCreatePurchases, canSubmitEmergencyPurchases, canApprovePurchasesFlag, canActOnPurchaseApprovalFlag, canReceivePurchases, canReceivePurchasesTeamFlag, canApprovePurchaseReceivingFlag, canInvoicePurchases, canPayMerchandisePurchasesFlag, canManageSupplierDebtPaymentsFlag] = await Promise.all([
    canSubmitPurchaseRequests(),
    // Confirmado 2026-09-22: pedido explícito del usuario — Bryan perdió
    // canSubmitPurchases del todo al pasar a liderar Marketing, y con eso
    // "Mis solicitudes" le desapareció incluyendo lo ya RECIBIDO hace
    // semanas. canViewOwnPurchaseHistory gatea la misma pestaña en modo
    // solo lectura para quien ya tiene historial propio (ver guards.ts).
  ...
  // Solicitud de Fulfillment (Yair) — vive dentro de "Registro de Egresos",
  // mismo patrón sin dept.code que el resto de este bloque.
  const [canSubmitFulfillmentReq, canViewFulfillmentReq] = await Promise.all([canSubmitFulfillmentRequest(), canViewFulfillmentRequests()]);
  ...
  // real — mismo patrón de visibilidad que supplierExchangeMineCount arriba.
  const canManagePurchaseGestionFlag = await canManageOutflowPurchaseGestion();
  const purchaseGestionPendingCount = canManagePurchaseGestionFlag ? await getPurchaseGestionPendingCount() : 0;
  // Guías Canceladas (Fase 4), rediseñado 2026-09-02: Bryan gestiona el
  ...
  const canRegisterLunchPayments = await checkCanRegisterLunchPayments();
  ...
  // propia "Mi área de trabajo" de quien lidera este departamento.
  const canManageImprovementPlanFlag = await canManageImprovementPlan(dept.id);
  ...
  return (
    <div>
      ...
      <DeptWorkspaceTabs
        ...
        canSubmitEmergencyPurchases={canSubmitEmergencyPurchases}
        ...
        canSubmitFulfillmentRequest={canSubmitFulfillmentReq}
        canViewFulfillmentRequests={canViewFulfillmentReq}
        ...
        canManagePurchaseGestion={canManagePurchaseGestionFlag}
        ...
        canActOnComboSuggestions={canActOnComboSuggestions}
        canMarkComboCreatedInDropi={canMarkComboCreatedInDropi}
        canProposeMarketProduct={canProposeMarketProductFlag}
