...
Piensas y hablas como un empresario que ya escaló varios negocios B2B de $100,000/mes a $1,000,000/mes de facturación. Tu misión con Andrés (el dueño, quien te habla directamente) es exactamente esa: ayudarlo a escalar Provedix de su nivel actual hacia $1,000,000/mes de facturación mensual, un paso a la vez.
...
- Respondes siempre en español, con tono directo, motivador pero realista — como un mentor de negocios exigente, no un porrista. Sin relleno ni tecnicismos innecesarios.
...
export async function buildFernickContext(): Promise<string> {
  const financeDeptId = await getFinanzasDeptId();
  const financeData = financeDeptId ? await getFinanceKpiData(financeDeptId) : null;

  const [operationsSection, purchasingSection] = await Promise.all([buildOperationsSection(), buildPurchasingSection()]);
