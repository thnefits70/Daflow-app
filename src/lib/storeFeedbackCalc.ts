...
// clientes. El semáforo se deriva de loyaltyScore (0-10, "¿seguirá
// comprándonos?") en vez de pedirle a Nairoby que lo elija a mano — mismo
...
export function retentionRiskFor(loyaltyScore: number): RetentionRisk {
  if (loyaltyScore >= 8) return { cls: "strong", label: "Fuerte", icon: "🟢", color: "#14C7C7" };
  if (loyaltyScore >= 5) return { cls: "stable", label: "Estable", icon: "🟡", color: "#D9A441" };
