...
export type CompiledFulfillmentLine = { catalogItemId: string; name: string; photos: string[]; quantity: number };
...
export async function getCompiledBatch(batchId: string): Promise<{ id: string; source: string; requestedAt: Date; requestedByName: string; totalRows: number; skippedCount: number; lines: CompiledFulfillmentLine[] } | null> {
  ...
  for (const item of batch.items) {
    ...
    else byItem.set(item.catalogItemId, { catalogItemId: item.catalogItemId, name: item.catalogItem.name, photos: item.catalogItem.photos, quantity: item.quantity });
  }
