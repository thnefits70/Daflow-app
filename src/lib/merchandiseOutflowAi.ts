...
export type OutflowManifestRow = { name: string; quantity: number };
...
export async function readOutflowManifest(params: { photoUrls: string[]; documentKind: "despacho" | "garantía"; actorId: string; deptId?: string }): Promise<OutflowManifestReadResult> {
  ...
  const response = await client.messages.create({
    ...
    system:
      ...
      'Responde ÚNICAMENTE un JSON: {"rows": [{"name": string, "quantity": number}]}. ' +
      "name es el nombre del producto tal como está escrito en el documento (no lo traduzcas ni lo normalices). " +
      "quantity es el número de unidades de ese renglón. Si no se distingue con claridad algún renglón, omítelo " +
      "de la lista en vez de adivinar.",
    messages: [
      {
        role: "user",
  ...
  return { rows: Array.isArray(result.rows) ? result.rows.filter((r) => r.name && r.quantity > 0) : [] };
}
