...
const FEATURE_LABELS: Record<AiUsageFeature, string> = {
  ...
  control_compras_revision_factura: "Control de Compras · revisión de la factura al cerrar",
  ...
  combo_sugerencias_match: "Sugerencias de Combos · filtrar parejas con sentido real",
  proveedor_credito_pago: "Proveedores con Crédito · revisión de tanda de pago",
  plan_mejora_redaccion: "Plan de Mejora · redactar plan",
...
export async function getAiSpendOverview(): Promise<AiSpendOverview> {
  ...
  const actorName = (actorId: string) =>
    actorId === "admin"
      ? "Andrés (admin)"
      : actorId === "system"
        ? "Automático (nicho al crear producto)"
        : actorId === "cron"
          ? "Automático (revisión diaria de nicho)"
          : (userNameById.get(actorId) ?? "Usuario eliminado");
